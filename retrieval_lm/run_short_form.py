...
from metrics import exact_match_score,numerical_accuracy, qa_f1_score, metric_max_over_ground_truths, evaluate_finqa
...
def process_data_evidences(demonstration, top_n):
    ...
    # Combine all passages into a single paragraph string
    paragraph = "\n".join([ctx["text"] for ctx in demonstration[ctx_key][:top_n]])
    ...
    evidences = demonstration.get(ctx_key, [])[:top_n]
    return prompt, evidences
...
def main():
    ...
    for i, row in tqdm(enumerate(input_data)):
        ...
        if args.task == "finqa":
            metric_result = evaluate_finqa(pred, row["answers"], tolerance=1e-3)
        ...
        if i % 10 == 0:
            em_avg = np.mean([m["em"] for m in metric_results])
            f1_avg = np.mean([m["f1"] for m in metric_results])
            print(f"Processed {i} rows. EM: {em_avg:.2f}, F1: {f1_avg:.2f}")
            final_results = {
                    "preds": preds,
                    "prompts": prompts,
                    ...
                    "metric_mean": {
                        "em": em_avg,
                        "f1": f1_avg,
                        "numerical_accuracy": np.mean([m.get("numerical_accuracy", 0) for m in metric_results]),
                    },
                    "scores": scores,
                }
    ...
    "golds": golds,
    "metric_mean": {
        "em": np.mean([m["em"] for m in metric_results]),
        "f1": np.mean([m["f1"] for m in metric_results]),
        "numerical_accuracy": np.mean([m["numerical_accuracy"] for m in metric_results]),
    },
    "scores": scores,}
